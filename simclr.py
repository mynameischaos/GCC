...
def main():
    ...
    aug_feat_memory = AugFeat('./aug_feat_memory', 9)
    org_feat_memory = AugFeat('./org_feat_memory', 9)
    ...
    for epoch in range(start_epoch, p['epochs']):
        ...
        # Train
        print('Train ...')
        simclr_train(train_dataloader, model, criterion, optimizer, epoch, aug_feat_memory, org_feat_memory)

        # Fill memory bank
        print('Fill memory bank mean for kNN...')
        fill_memory_bank_mean(base_dataloader, aug_feat_memory, org_feat_memory,  memory_bank_base)
        ...
        if (epoch + 1) % 5 == 0:
            pass
            #head = state_dict['head'] if p['setup'] == 'scan' else 0
            #predictions, features = get_predictions(p, dataloader, model, return_features=True)
