...
def get_predictions(p, dataloader, model, return_features=False, self_labeling=False):
    ...
    model.eval()
    predictions = [[] for _ in range(p['model_kwargs']['nheads'])]
    probs = [[] for _ in range(p['model_kwargs']['nheads'])]
    targets = []
    if return_features:
    ...
    for batch in dataloader:
        ...
        res = model(images)
        if self_labeling:
            output = res # self labeling
        else:
            output = res[1]
        if return_features:
            features[ptr: ptr+bs] = res[0]
            ptr += bs
        for i, output_i in enumerate(output):
            predictions[i].append(torch.argmax(output_i, dim=1))
            probs[i].append(F.softmax(output_i, dim=1))
        targets.append(batch['target'])
        index.append(batch['meta']['index'])
    ...
    if return_features:
        return out, features.cpu(), targets.cpu()
    else:
        return out
